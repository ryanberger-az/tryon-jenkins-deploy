...
$wc.DownloadFile("https://github.com/winsw/winsw/releases/download/v2.8.0/WinSW.NET2.exe", $wrapperExec)
$wc.DownloadFile("https://raw.githubusercontent.com/ryanberger-az/tryon-jenkins-deploy/master/other/jenkins-slave.exe.config", "c:\jenkins\jenkins-slave.exe.config")
$wc.DownloadFile("https://raw.githubusercontent.com/ryanberger-az/tryon-jenkins-deploy/master/other/jenkins-slave.xml", $configFile)

# Prepare config
Write-Output "Executing agent process "
$configExec = "${javaHome}\bin\java.exe"
$configArgs = "-jar c:\jenkins\slave.jar -jnlpUrl `"${jenkinsserverurl}/computer/${vmname}/slave-agent.jnlp`" -noReconnect"
if ($secret) {
